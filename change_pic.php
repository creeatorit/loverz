...
function changeProfilePic() {
	...
	if(strlen($name)) {
		...
		if(in_array($ext,$valid_formats)) {
			if($size<(1024*1024)) {
				$actual_image_name = 'avatar' .'_'.substr(uniqid(rand()), 0, 5) .'.'.$ext;
			...
			echo "Tamanho máximo de 1 MB"; 
		}
		else
		echo "Formatos permitidos(jpg, png, gif, jpeg)"; 
	}
	else
...
function saveProfilePic(){
	...
	if($post['id']){
		$sql_query = "SELECT * FROM usuarios WHERE id = '".mysqli_escape_string($conn, $post['id'])."'";		
		...
		if(mysqli_num_rows($resultset)) {                     
			...
			$sql_update = 'UPDATE usuarios set foto="'.mysqli_escape_string($conn,$post['image_name']).'" WHERE id="'.$_SESSION['UsuarioID'].'" ';
